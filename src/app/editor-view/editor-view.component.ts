...
export class EditorViewComponent implements OnInit {
  ...
  ngOnInit() {
    this.dataService.currentActiveElementID.subscribe((id) => {
      //if the user wants to see different elements e.g. the children of one
  ...
  onHoveredParentElementIDChange(parentElementID: string | null) {
    //to pass the hovered Element from editorpart to navigationpart
  ...
  updateEditorView() {
    // gives editorpart and navigationpart the new currentElement so they show it
